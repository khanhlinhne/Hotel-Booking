...
public class LoginController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        User user = new UserDBContext().getUserbyUsername(acc);
        // Check if the username and password match (replace with your authentication logic)
        if (new AccountDBContext().getAccount(acc) != null) {
            // Create a session
            HttpSession session = request.getSession();
            session.setAttribute("user", user);
            session.setAttribute("acc", new AccountDBContext().getAccount(acc));
            response.sendRedirect("homepage"); // Redirect to a welcome page after successful login
        } else {
